...
describe('SignalService', () => {
  it('creates a signal from a raw item and preserves the raw item reference', async () => {
    const repository = {
      findByRawItemAndType: jest.fn(() => Promise.resolve(null)),
  ...
  it('updates an existing signal for the same raw item and signal type', async () => {
    const repository = {
      findByRawItemAndType: jest.fn(() =>
        Promise.resolve({
          id: 'sig_1',
          rawItemId: 'raw_1',
          source: 'x',
      ...
      update: jest.fn((id, input) => ({
        ...
        metadata: input.metadata,
        createdAt: new Date('2026-08-24T10:10:00.000Z'),
        updatedAt: new Date('2026-08-24T10:20:00.000Z'),
      })),
      create: jest.fn(),
    } as unknown as SignalRepository;
    const service = new SignalService(repository);
