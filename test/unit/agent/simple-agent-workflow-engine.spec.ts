...
import { SimpleAgentWorkflowEngine } from '../../../src/agent/workflow-engine/simple-agent-workflow-engine';
import { ToolExecutorService } from '../../../src/agent/tool-registry/tool-executor.service';

describe('SimpleAgentWorkflowEngine', () => {
  it('finishes successfully when the model returns a final decision', async () => {
    ...
    const engine = new SimpleAgentWorkflowEngine(model, toolExecutor, runLog);
  ...
  it('executes tools before returning a final decision', async () => {
    const model = new MockModelProvider([
      {
        type: 'tool_call',
