  ...
  private createAgentContract(agentType: string): string {
    const contracts: Record<string, string> = {
      ...
      assistant: [
        ...
        '配置编辑时，proposedActions.tool 只能使用这些后端受控工具名：update_twitter_config、set_twitter_trend_schedule。',
        '数据查询：只有用户明确询问信号、事件、证据、机会、最近数据、为什么没有形成事件等运行数据时，才调用 signal/event/evidence/opportunity 等数据工具。',
